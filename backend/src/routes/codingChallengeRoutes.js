...
const execLimiter = rateLimit({
  ...
  message: { success: false, message: 'Too many code executions — slow down and try again in a minute' },
});
...
router.post(
  ...
  [
    ...
    body('testCases').isArray({ min: 1 }).withMessage('At least 1 test case is required'),
  ],
  validate,
...
router.get('/:id/submissions', authorize('mentor', 'admin'), ctrl.listSubmissions);
