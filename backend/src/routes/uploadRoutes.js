...
router.post(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    const url = buildFileUrl(req, req.file, "avatars");

    // Delete old avatar (disk storage only — see deleteFile in uploadService)
    if (req.user.avatar && uploadDir) {
...
router.post(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    return ApiResponse.success(res, 200, "Document uploaded", {
      url: buildFileUrl(req, req.file, "documents"),
...
router.post(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    const files = req.files.map((f) => ({
      url: buildFileUrl(req, f, "submissions"),
