...
exports.forgotPassword = asyncHandler(async (req, res, next) => {
  ...
  if (!emailed) {
    if (process.env.NODE_ENV === 'production') {
      logger.warn(`Password reset requested for ${user.email} but no email was sent (SMTP not configured). Token not returned to client.`);
    } else {
  ...
  return ApiResponse.success(res, 200, 'If that email exists, reset instructions have been sent', {
    emailed,
    resetToken: !emailed && process.env.NODE_ENV !== 'production' ? resetToken : undefined,
  });
});
