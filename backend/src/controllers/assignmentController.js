...
const assertTeachesBatch = async (batchId, req, next) => {
  ...
  if (!batch || String(batch.mentor) !== String(req.user._id)) {
    next(new AppError('Forbidden — you can only manage assignments for your own batches', 403));
    return false;
  }
  return true;
};
...
exports.updateAssignment = asyncHandler(async (req, res, next) => {
  ...
  const {
    title, description, dueDate, maxScore, instructions, attachments,
    status, type, difficulty, referenceLinks,
  } = req.body;
  const updates = {
    title, description, dueDate, maxScore, instructions, attachments,
    status, type, difficulty, referenceLinks,
...
exports.reviewSubmission = asyncHandler(async (req, res, next) => {
  ...
  if (score != null && (Number.isNaN(Number(score)) || score < 0 || score > maxScore)) {
