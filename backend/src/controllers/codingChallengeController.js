...
const assertTeachesBatch = async (batchId, req, next) => {
  ...
  if (!batch || String(batch.mentor) !== String(req.user._id)) {
    next(
      new AppError(
        "Forbidden — you can only manage challenges for your own batches",
        403,
      ),
    );
    return false;
...
exports.getChallenge = asyncHandler(async (req, res, next) => {
  ...
  if (req.user.role === "student") {
    return ApiResponse.success(res, 200, "Challenge fetched", {
      challenge: challenge.toStudentView(),
    });
  }
...
exports.runCode = asyncHandler(async (req, res, next) => {
  const challenge = await CodingChallenge.findById(req.params.id).select(
    "batch allowedLanguages status",
  );
  if (!challenge) return next(new AppError("Challenge not found", 404));
  ...
  if (!challenge.allowedLanguages.includes(language)) {
    return next(
      new AppError(
        `Language must be one of: ${challenge.allowedLanguages.join(", ")}`,
        400,
      ),
    );
  }
...
exports.submitCode = asyncHandler(async (req, res, next) => {
  ...
  if (challenge.status !== "published")
    return next(new AppError("This challenge is not currently open", 400));
  ...
  const existingAttempts = await CodingSubmission.countDocuments({
  ...
  if (existingAttempts >= challenge.maxAttempts) {
    return next(
      new AppError(
        `Maximum attempts (${challenge.maxAttempts}) already used`,
        400,
      ),
    );
  }
...
exports.mySubmissions = asyncHandler(async (req, res) => {
  ...
  return ApiResponse.success(res, 200, "Your submissions fetched", {
    submissions,
  });
});
