...
exports.payMyFees = asyncHandler(async (req, res, next) => {
  ...
  let method = String(req.body.method || "upi")
    .toLowerCase()
    .replace(/\s|\//g, "");
  if (method === "netbanking" || method === "banktransfer")
  ...
  if (enrollmentId) {
    ...
    enrollment = enrollments.sort(
      (a, b) => (b.fee.due || 0) - (a.fee.due || 0),
    )[0];
  }
  if (!enrollment)
    return next(new AppError("No enrollment found to pay for", 404));

  if (enrollment.fee.due != null && amount > enrollment.fee.due)
    amount = enrollment.fee.due;
  if (amount <= 0)
    return next(new AppError("No pending dues on this course", 400));
  ...
  const payment = await Payment.create({
    ...
    method,
    notes:
      "Self-reported by student — pending admin verification (no payment gateway integrated)",
    status: "pending",
  });
...
exports.updatePayment = asyncHandler(async (req, res, next) => {
  ...
  if (notes !== undefined)
    payment.notes = reason ? `${notes} (${reason})` : notes;
  ...
  if (!wasCompleted && payment.status === "completed") {
    ...
    if (enrollment) {
      ...
      enrollment.fee.due = Math.max(
        0,
        enrollment.fee.total - enrollment.fee.paid,
      );
      enrollment.fee.payments.push({
...
exports.refundPayment = asyncHandler(async (req, res, next) => {
  ...
  if (payment.status === "refunded")
    return next(new AppError("Already refunded", 400));
...
exports.mySummary = asyncHandler(async (req, res) => {
  ...
  const totalCommitted = enrollments.reduce(
    (s, e) => s + (e.fee.total || 0),
    0,
  );
