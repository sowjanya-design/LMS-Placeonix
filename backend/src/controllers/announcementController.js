...
exports.createAnnouncement = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const roles =
      aud.roles && aud.roles.length ? aud.roles : ["student", "mentor"];
    ...
    logger.error(
      `Announcement notification fan-out failed for ${announcement._id}: ${e.message}`,
    );
  }
...
exports.markAsRead = asyncHandler(async (req, res, next) => {
  ...
  const already = announcement.readBy.find(
