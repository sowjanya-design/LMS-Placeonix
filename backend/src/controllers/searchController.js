...
exports.globalSearch = asyncHandler(async (req, res) => {
  ...
  const userFilter = {
    ...
    $or: [
      ...
      { lastName: rx },
      { email: rx },
      { "studentProfile.enrollmentId": rx },
    ],
  };
  ...
  if (req.user.role === "mentor") {
    ...
    userFilter.$and = [
      { $or: [{ _id: { $in: myStudentIds } }, { _id: req.user._id }] },
    ];
  }
  const [users, courses, batches, drives] = await Promise.all([
    User.find(userFilter)
      .limit(6)
      .select("firstName lastName email role studentProfile.enrollmentId"),
    Course.find({ title: rx }).limit(5).select("title category"),
    Batch.find({ $or: [{ name: rx }, { code: rx }] })
      .limit(5)
      .select("name code"),
    PlacementDrive.find({ $or: [{ company: rx }, { role: rx }] })
      .limit(5)
      .select("company role"),
  ]);
