...
const assertTeachesBatch = async (batchId, req, next) => {
  ...
  if (!batch || String(batch.mentor) !== String(req.user._id)) {
    next(new AppError('Forbidden — you can only manage quizzes for your own batches', 403));
    return false;
  }
  return true;
};
...
exports.listQuizzes = asyncHandler(async (req, res) => {
  ...
  return ApiResponse.paginated(res, 'Quizzes fetched', quizzes, page, limit, total);
});

// @desc   Get quiz (correct answers stripped for students)
// @route  GET /api/v1/quizzes/:id
exports.getQuiz = asyncHandler(async (req, res, next) => {
  const quiz = await Quiz.findById(req.params.id).populate('course batch createdBy');
  if (!quiz) return next(new AppError('Quiz not found', 404));

  if (req.user.role === 'student') {
    return ApiResponse.success(res, 200, 'Quiz fetched', { quiz: quiz.toStudentView() });
  }
...
exports.updateQuiz = asyncHandler(async (req, res, next) => {
  ...
  const {
    title, description, questions, timeLimitMinutes, maxAttempts,
    passingScorePercent, availableFrom, availableUntil, status,
  } = req.body;
  const updates = {
    title, description, questions, timeLimitMinutes, maxAttempts,
    passingScorePercent, availableFrom, availableUntil, status,
...
exports.startAttempt = asyncHandler(async (req, res, next) => {
  ...
  const enrolled = await Enrollment.findOne({ student: req.user._id, batch: quiz.batch });
  if (!enrolled) return next(new AppError('You are not enrolled in this batch', 403));
  if (!quiz.isOpen) return next(new AppError('This quiz is not currently open', 400));

  const existingAttempts = await QuizResult.countDocuments({ quiz: quiz._id, student: req.user._id });
  if (existingAttempts >= quiz.maxAttempts) {
    return next(new AppError(`Maximum attempts (${quiz.maxAttempts}) already used`, 400));
  }
  const inProgress = await QuizResult.findOne({ quiz: quiz._id, student: req.user._id, status: 'in_progress' });
  if (inProgress) {
  ...
  return ApiResponse.created(res, 'Attempt started', { attempt, quiz: quiz.toStudentView() });
});
...
exports.submitAttempt = asyncHandler(async (req, res, next) => {
  ...
  const answerByQuestion = new Map(submitted.map((a) => [String(a.question), a.selectedOptions || []]));
...
exports.myResults = asyncHandler(async (req, res) => {
  const results = await QuizResult.find({ quiz: req.params.id, student: req.user._id }).sort('attemptNumber');
