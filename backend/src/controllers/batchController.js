...
exports.getBatch = asyncHandler(async (req, res, next) => {
  ...
  const enrollments = await Enrollment.find({ batch: batch._id }).populate(
    "student",
    "firstName lastName email avatar studentProfile.enrollmentId",
  );
...
exports.deleteBatch = asyncHandler(async (req, res, next) => {
  ...
  if (enrollmentCount > 0) {
    return next(
      new AppError("Cannot delete batch with active enrollments", 400),
    );
  }
...
exports.enrollStudent = asyncHandler(async (req, res, next) => {
  ...
  if (existing)
    return next(new AppError("Already enrolled in this batch", 409));
  ...
  await Course.findByIdAndUpdate(batchDoc.course._id, {
  ...
  try {
    ...
    logger.error(
      `Enrollment email failed for ${student.email}: ${err.message}`,
    );
  }
...
exports.bulkEmail = asyncHandler(async (req, res, next) => {
  ...
  if (recipients.length === 0) {
    return next(
      new AppError(
        "This batch has no enrolled students with an email on file",
        400,
      ),
    );
  }
  ...
  for (const e of recipients) {
    try {
      await sendEmail({
        ...
        html: baseTemplate(
          subject,
          `<p>Hi ${e.student.firstName || ""},</p>${body}`,
        ),
      });
      sent += 1;
    } catch (err) {
  ...
  return ApiResponse.success(
    res,
    200,
    `Sent to ${sent} of ${recipients.length} students`,
