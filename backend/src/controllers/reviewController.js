...
exports.markHelpful = asyncHandler(async (req, res, next) => {
  // Only increments the first time a given user marks a given review helpful —
  // findOneAndUpdate's filter excludes users already in helpfulBy, so repeat
  // calls from the same account are a no-op instead of inflating the count.
