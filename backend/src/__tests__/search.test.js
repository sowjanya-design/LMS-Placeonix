...
describe("Search API — mentor scoping", () => {
  ...
  it("does not let a mentor find a student outside their own batches", async () => {
    const { user: mentor, token: mentorToken } = await createUserAndLogin({
      role: "mentor",
    });
    const { user: admin } = await createUserAndLogin({ role: "admin" });
    const { user: myStudent } = await createUserAndLogin({
      role: "student",
      firstName: "Zaphira",
    });
    const { user: otherStudent } = await createUserAndLogin({
      role: "student",
      firstName: "Zorlana",
    });
    ...
    expect(
      resMine.body.data.results.some((r) => r.id === String(myStudent._id)),
    ).toBe(true);
    ...
    expect(
      resOther.body.data.results.some((r) => r.id === String(otherStudent._id)),
    ).toBe(false);
  });
  ...
  it("lets an admin find any student", async () => {
    ...
    expect(
      res.body.data.results.some((r) => r.id === String(student._id)),
    ).toBe(true);
  });
});
