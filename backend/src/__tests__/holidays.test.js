...
describe("Holiday auto-population", () => {
  ...
  it("creates a holiday announcement for every curated holiday, once an admin exists", async () => {
    ...
    const expectedCount =
      getIndiaHolidays(currentYear).length +
      getIndiaHolidays(currentYear + 1).length;
  ...
  it("lists a future-dated holiday even though its publishAt is in the future", async () => {
    ...
    const res = await request(app)
      .get("/api/v1/announcements?type=holiday&limit=100")
      .set(auth(token));
    expect(res.statusCode).toBe(200);
    expect(res.body.data.length).toBeGreaterThan(0);

    const now = new Date();
    const hasFutureHoliday = res.body.data.some(
      (a) => new Date(a.publishAt) > now,
    );
