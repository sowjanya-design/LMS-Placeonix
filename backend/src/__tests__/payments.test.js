...
describe('Payments API — self-report integrity + access scope', () => {
  ...
  it('creates a student-submitted payment as pending, never completed, and does not touch the enrollment balance', async () => {
    ...
    const { user: student, token } = await createUserAndLogin({ role: 'student' });
    const { batch, course } = await createCourseAndBatch(mentor._id, mentor._id);
    const enrollment = await Enrollment.create({
      student: student._id, course: course._id, batch: batch._id,
  ...
  it('blocks a mentor from listing all students\' payments (route-level authorize)', async () => {
    ...
    const res = await request(app).get('/api/v1/payments').set(auth(mentorToken));
  ...
  it('scopes a student\'s payment list to their own records only', async () => {
    ...
    const { batch, course } = await createCourseAndBatch(mentor._id, mentor._id);
    const enrollA = await Enrollment.create({ student: studentA._id, course: course._id, batch: batch._id, fee: { total: 100, paid: 0, due: 100 } });
    const enrollB = await Enrollment.create({ student: studentB._id, course: course._id, batch: batch._id, fee: { total: 100, paid: 0, due: 100 } });
    await Payment.create({ enrollment: enrollA._id, student: studentA._id, amount: 50, method: 'upi', status: 'completed' });
    await Payment.create({ enrollment: enrollB._id, student: studentB._id, amount: 50, method: 'upi', status: 'completed' });

    const res = await request(app).get('/api/v1/payments').set(auth(tokenA));
    expect(res.statusCode).toBe(200);
    expect(res.body.data.length).toBe(1);
    expect(String(res.body.data[0].student._id || res.body.data[0].student)).toBe(String(studentA._id));
  });
});
