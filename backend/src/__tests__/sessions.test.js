...
describe('Sessions API — ownership enforcement', () => {
  ...
  it('lets a mentor create and update their own session', async () => {
    ...
    const createRes = await request(app)
      ...
      .set(auth(token))
      .send({ batch: batch._id, title: 'Intro', startTime: new Date(), endTime: new Date(Date.now() + 3600000) });
  ...
  it('blocks a different mentor from updating, deleting, starting, or completing a session they do not own', async () => {
    ...
    const attempts = [
      request(app).patch(`/api/v1/sessions/${session._id}`).set(auth(intruderToken)).send({ title: 'Hijacked' }),
      request(app).delete(`/api/v1/sessions/${session._id}`).set(auth(intruderToken)),
      request(app).patch(`/api/v1/sessions/${session._id}/start`).set(auth(intruderToken)),
      request(app).patch(`/api/v1/sessions/${session._id}/complete`).set(auth(intruderToken)).send({}),
    ];
  ...
  it('rejects an invalid meetingLink format', async () => {
    ...
    const res = await request(app)
      .post('/api/v1/sessions')
      .set(auth(token))
      .send({ batch: batch._id, title: 'Bad link', startTime: new Date(), endTime: new Date(Date.now() + 3600000), meetingLink: 'not-a-url' });
