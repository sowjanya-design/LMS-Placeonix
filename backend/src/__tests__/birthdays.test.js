...
describe('GET /api/v1/users/birthdays', () => {
  ...
  it('returns month/day for students with a DOB set, never the raw date or email', async () => {
    ...
    await createUserAndLogin({ role: 'student', firstName: 'No', lastName: 'Birthday' });
    const { token } = await createUserAndLogin({ role: 'student' });

    const res = await request(app).get('/api/v1/users/birthdays').set(auth(token));
  ...
  it('does not shadow GET /users/:id — an id-like path still resolves normally', async () => {
    const { user, token: adminToken } = await createUserAndLogin({ role: 'admin' });
    const res = await request(app).get(`/api/v1/users/${user._id}`).set(auth(adminToken));
    expect(res.statusCode).toBe(200);
  });
});
