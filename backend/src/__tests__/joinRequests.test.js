...
describe("Join Requests API — ownership enforcement", () => {
  ...
  it("lets a student create a join request for their batch's mentor", async () => {
    ...
    const { token: studentToken } = await createUserAndLogin({
      role: "student",
    });
    ...
    expect(
      res.body.data.mentor.toString
        ? res.body.data.mentor.toString()
        : res.body.data.mentor,
    ).toBe(String(mentor._id));
  });
  ...
  it("scopes list() to a mentor's own requests and a student's own requests", async () => {
    const { user: mentorA, token: tokenA } = await createUserAndLogin({
      role: "mentor",
    });
    ...
    const { batch: batchA } = await createCourseAndBatch(
      mentorA._id,
      mentorA._id,
    );
    const { batch: batchB } = await createCourseAndBatch(
      mentorB._id,
      mentorB._id,
    );
    ...
    await request(app)
      .post("/api/v1/join-requests")
      .set(auth(studentToken))
      .send({ batchId: batchA._id, reason: "a" });
    await request(app)
      .post("/api/v1/join-requests")
      .set(auth(studentToken))
      .send({ batchId: batchB._id, reason: "b" });
