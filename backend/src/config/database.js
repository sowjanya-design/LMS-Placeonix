...
const connectDB = async () => {
  ...
  if (!uri) {
    ...
    logger.error(
      "  → Create placeonix-hub-backend/.env (copy from .env.example) and set MONGO_URI.",
    );
    logger.error("    Local MongoDB:  mongodb://localhost:27017/placeonix-hub");
    logger.error(
      "    MongoDB Atlas:  mongodb+srv://<user>:<pass>@<cluster>.mongodb.net/placeonix-hub",
    );
    process.exit(1);
  }
  ...
  try {
    ...
    logger.error(
      "  Most likely MongoDB is not running / not reachable. Fix one of:",
    );
    logger.error(
      "  1) Start a local MongoDB (mongod) so mongodb://localhost:27017 works, OR",
    );
    logger.error(
      "  2) Set MONGO_URI in .env to a MongoDB Atlas connection string (free tier).",
    );
    logger.error(
      `  Current MONGO_URI host: ${(uri.split("@")[1] || uri).split("/")[0]}`,
    );
    process.exit(1);
  }
};
