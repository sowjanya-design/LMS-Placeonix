...
exports.updateRolePermissions = asyncHandler(async (req, res, next) => {
  ...
  if (!Array.isArray(permissions)) {
    return next(new AppError('permissions must be an array of permission codes', 400));
  }

  const validCodes = new Set((await Permission.find().select('code')).map((p) => p.code));
  const invalid = permissions.filter((p) => !validCodes.has(p));
  if (invalid.length) {
    return next(new AppError(`Unknown permission code(s): ${invalid.join(', ')}`, 400));
  }
  ...
  if (role.code === 'admin') {
    return next(new AppError("The 'admin' role's permissions cannot be edited — it always has full access", 400));
  }
  ...
  return ApiResponse.success(res, 200, `Permissions updated for role '${role.code}'`, { role });
});
