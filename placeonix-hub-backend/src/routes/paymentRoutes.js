...
router.get('/:id', authorize('student', 'admin'), [param('id').isMongoId()], validate, ctrl.getPayment);
...
router.post(
  ...
  [
    ...
    body('amount').isFloat({ gt: 0 }).withMessage('amount must be a positive number'),
    body('method').isIn(methodEnum).withMessage(`method must be one of: ${methodEnum.join(', ')}`),
...
router.post(
  ...
  [
    ...
    body('reason').optional().isString().trim().isLength({ max: 500 }),
    body('refundTransactionId').optional().isString().trim().isLength({ max: 200 }),
  ],
  validate,
  ctrl.refundPayment
);
