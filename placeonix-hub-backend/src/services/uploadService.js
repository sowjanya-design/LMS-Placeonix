...
const s3Configured = !!(process.env.S3_BUCKET && process.env.AWS_ACCESS_KEY_ID && process.env.AWS_SECRET_ACCESS_KEY);
...
if (s3Configured) {
  ...
  storage = (subdir) =>
    multerS3({
      ...
      key: (req, file, cb) => cb(null, `${subdir}/${safeFilename(file.originalname)}`),
    });
  ...
  uploadDir = process.env.FILE_UPLOAD_PATH || (isServerless ? '/tmp/uploads' : './uploads');
  try {
