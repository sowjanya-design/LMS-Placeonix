 ...
 */
// Tracks consecutive failures so a real outage (DB down, bad schema) escalates
// past a single quiet log line instead of silently dropping every notification
// — e.g. fee reminders or assignment-created pings — with nothing surfacing it.
...
class NotificationService {
  static async send(options) {
    try {
      const result = await Notification.notify(options);
      ...
      if (consecutiveFailures >= FAILURE_ESCALATION_THRESHOLD) {
        logger.error(
          `ALERT: ${consecutiveFailures} consecutive notification failures — ` +
          `students are very likely not receiving fee reminders, attendance warnings, or assignment updates. Investigate immediately.`
        );
      }
      return null;
    }
