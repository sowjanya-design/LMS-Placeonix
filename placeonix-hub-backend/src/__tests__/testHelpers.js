...
async function createUserAndLogin(overrides = {}) {
  const email = overrides.email || `u${Date.now()}${Math.random().toString(36).slice(2)}@test.com`;
  ...
  const token = generateAccessToken({ id: user._id, role: user.role, email: user.email });
  return { user, token };
}
