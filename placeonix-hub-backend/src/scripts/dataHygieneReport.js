...
async function findSuspiciousUsers() {
  const candidates = await User.find({ role: { $in: ['student', 'mentor'] } })
    .select('firstName lastName email role status');
  return candidates.filter((u) => {
    const fullName = `${u.firstName || ''} ${u.lastName || ''}`.trim();
    return isSuspiciousName(u.firstName) || isSuspiciousName(fullName) || TEST_EMAIL_RX.test(u.email || '');
  });
}
...
async function findDuplicateNames() {
  const agg = await User.aggregate([
    ...
    { $group: { _id: { firstName: '$firstName', lastName: '$lastName', role: '$role' }, count: { $sum: 1 }, ids: { $push: '$_id' } } },
    { $match: { count: { $gt: 1 } } },
  ]);
  return agg;
}
...
async function findSuspiciousBatches() {
  ...
  return batches.filter((b) => isSuspiciousName(b.name) || isSuspiciousName(b.code));
}
...
async function findSuspiciousAnnouncements() {
  ...
  return anns.filter((a) => isSuspiciousName(a.title) || (a.body || '').trim().length < 6);
}

async function run() {
  await connectDB();

  const [users, dupes, batches, companies, leads, announcements] = await Promise.all([
    findSuspiciousUsers(),
    findDuplicateNames(),
  ...
  users.forEach((u) => logger.info(`  [User ${u.role}] ${u.firstName} ${u.lastName} <${u.email}> status=${u.status}`));

  logger.info(`Duplicate mentor/student name groups: ${dupes.length}`);
  dupes.forEach((d) => logger.info(`  [Duplicate] ${d._id.firstName} ${d._id.lastName} (${d._id.role}) x${d.count} — ids: ${d.ids.join(', ')}`));
  ...
  leads.forEach((l) => logger.info(`  [Lead] ${l.firstName} ${l.lastName} <${l.email}>`));

  logger.info(`Suspicious announcements: ${announcements.length}`);
  announcements.forEach((a) => logger.info(`  [Announcement] "${a.title}" published=${a.isPublished}`));

  if (!APPLY) {
    logger.info('Dry run — nothing changed. Re-run with --apply to soft-flag these records, or --apply --hard-delete to remove them.');
  ...
  if (HARD_DELETE) {
    ...
    await Promise.all([
      ...
      Announcement.deleteMany({ _id: { $in: announcements.map((a) => a._id) } }),
    ]);
    logger.info('Hard delete complete.');
  } else {
    await Promise.all([
      User.updateMany({ _id: { $in: users.map((u) => u._id) } }, { $set: { status: 'suspended' } }),
      Announcement.updateMany({ _id: { $in: announcements.map((a) => a._id) } }, { $set: { isPublished: false } }),
    ]);
    logger.info('Soft-flagged suspicious users (status=suspended) and announcements (isPublished=false).');
    logger.info('Batches/Companies/Leads have no status field to soft-flag — review the report above and remove manually, or re-run with --hard-delete.');
  }
