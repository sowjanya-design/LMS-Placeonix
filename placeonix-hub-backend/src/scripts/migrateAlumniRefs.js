...
async function run() {
  ...
  let matchedCourse = 0, unmatchedCourse = 0;
  let matchedBatch = 0, unmatchedBatch = 0;
  const updates = [];
  ...
  for (const a of alumni) {
    ...
    if (!a.courseRef && a.course) {
      ...
      if (id) { patch.courseRef = id; matchedCourse += 1; }
      else unmatchedCourse += 1;
    }

    if (!a.batchRef && a.batch) {
      const id = batchByName.get(normalize(a.batch));
      if (id) { patch.batchRef = id; matchedBatch += 1; }
      else unmatchedBatch += 1;
    }

    if (Object.keys(patch).length) updates.push({ id: a._id, name: a.name, patch });
  }

  logger.info(`Alumni records scanned: ${alumni.length}`);
  logger.info(`Course matches: ${matchedCourse} matched, ${unmatchedCourse} unmatched (kept as free text)`);
  logger.info(`Batch matches: ${matchedBatch} matched, ${unmatchedBatch} unmatched (kept as free text)`);

  if (DRY_RUN) {
    logger.info('--dry-run: no writes performed. Re-run without the flag to apply.');
    updates.forEach((u) => logger.info(`  would update "${u.name}" (${u.id}): ${JSON.stringify(u.patch)}`));
  } else {
