...
export default function VerifyCertificatePage() {
  ...
  useEffect(() => {
    api
      .get<VerifyResponse>(
        `/certificates/verify/${encodeURIComponent(params.number)}`,
      )
      .then(setResult)
      .catch((err) =>
        setError(
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-bg px-4 py-12">
      ...
      <div
        className="w-full max-w-[460px] rounded-[28px] bg-white p-8"
        style={{ boxShadow: "var(--clay-shadow-card)" }}
      >
        {!result && !error && (
          <div className="py-6 text-center text-sm text-muted">
            Checking certificate {params.number}…
          </div>
        )}

        {error && (
          <div className="flex flex-col items-center gap-3 py-4 text-center">
            <div className="flex h-14 w-14 items-center justify-center rounded-full bg-amber-lt text-2xl">
              !
            </div>
        ...
        {result && !result.valid && (
          <div className="flex flex-col items-center gap-3 py-4 text-center">
            <div className="flex h-14 w-14 items-center justify-center rounded-full bg-red-lt text-2xl text-red">
              ✕
            </div>
            <h1 className="text-lg font-extrabold text-ink">
              Couldn&rsquo;t verify this certificate
            </h1>
            <p className="text-sm text-muted">
              <span className="font-mono">{params.number}</span> doesn&rsquo;t
              match an active Placeonix certificate. It may be mistyped, or the
        ...
        {result?.valid && result.certificate && (
          <div className="flex flex-col items-center gap-1 text-center">
            <div className="mb-2 flex h-14 w-14 items-center justify-center rounded-full bg-green-lt text-2xl text-green">
              ✓
            </div>
            <h1 className="text-lg font-extrabold text-ink">
            ...
            <p className="mb-5 text-sm text-muted">
              This is a genuine Placeonix certificate.
            </p>
            ...
            <div className="w-full divide-y divide-line rounded-2xl border border-line text-left">
              ...
              <Row
                label="Issued on"
                value={fmt(result.certificate.issuedDate)}
              />
              <Row
                label="Certificate No."
                value={result.certificate.number}
                mono
              />
            </div>
          </div>
      ...
      <Link
        href="/login"
        className="mt-6 text-sm font-bold text-purple hover:text-purple-dk"
      >
        Go to Placeonix Hub →
      </Link>
    </div>
...
function Row({
  ...
  return (
    <div className="flex items-center justify-between gap-4 px-4 py-3">
      ...
      <span
        className={`text-right text-sm ${bold ? "font-extrabold text-ink" : "font-semibold text-ink2"} ${mono ? "font-mono text-xs" : ""}`}
      >
        {value}
      </span>
    </div>
