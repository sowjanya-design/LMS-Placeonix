...
function TakeQuiz({
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const res = await api.post<{ attempt: QuizResult }>(
        `/quizzes/${quiz._id}/attempts/${attemptId}/submit`,
        payload,
      );
      onDone(res.attempt);
    } catch (err) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        ...
        <div
          className={`rounded-full px-4 py-2 text-sm font-bold ${secondsLeft < 60 ? "bg-red-lt text-red" : "bg-purple-lt text-purple"}`}
        >
                ...
                return (
                  <label
                    ...
                    className={`flex cursor-pointer items-center gap-2.5 rounded-lg border-[1.5px] px-3.5 py-2.5 text-sm transition-colors ${
                      selected
                        ? "border-purple bg-purple-lt text-purple font-semibold"
                        : "border-line text-ink2 hover:border-purple/40"
                    }`}
                  >
                    <input
...
function StudentQuizzes() {
    ...
    api
      ...
      .then(async (list) => {
        ...
        const entries = await Promise.all(
          list.map(async (q) => {
            try {
              const res = await api.get<{ results: QuizResult[] }>(
                `/quizzes/${q._id}/results/me`,
              );
  ...
  async function handleStart(quiz: Quiz) {
    setError(null);
    try {
      const res = await api.post<{ attempt: QuizResult; quiz: Quiz }>(
        `/quizzes/${quiz._id}/attempts`,
      );
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-sm text-muted">
          Test your knowledge across your enrolled batches.
        </p>
      </div>
      ...
      {justSubmitted && (
        <div className="rounded-[14px] border-2 border-purple bg-purple-lt p-5 text-center">
          <p className="text-lg font-extrabold text-purple">
            You scored {justSubmitted.score}/{justSubmitted.maxScore} (
            {justSubmitted.percentage}%)
          </p>
          <p className="text-sm font-semibold text-ink2">
            {justSubmitted.passed
              ? "🎉 You passed!"
              : "Didn't clear the passing score this time."}
          </p>
        </div>
      )}

      {quizzes && quizzes.length === 0 && (
        <EmptyState message="No quizzes assigned yet." />
      )}
        ...
        <div className="flex flex-col gap-3">
          {quizzes.map((q) => {
            ...
            const best = submitted.reduce(
              (b, r) => (!b || r.percentage > b.percentage ? r : b),
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-xs font-medium tracking-wide text-muted uppercase">
                      ...
                      {q.batch?.name || "Unknown Batch"}
                    </p>
                    <h2 className="font-bold text-ink">{q.title}</h2>
                    <p className="mt-1 text-xs text-muted">
                      {q.questions?.length ?? "?"} questions ·{" "}
                      {q.timeLimitMinutes} min · {attemptsLeft} of{" "}
                      {q.maxAttempts} attempts left
                    </p>
                  </div>
...
function QuizModal({
  ...
  const [timeLimitMinutes, setTimeLimitMinutes] = useState(
    String(quiz?.timeLimitMinutes ?? 30),
  );
  const [maxAttempts, setMaxAttempts] = useState(
    String(quiz?.maxAttempts ?? 1),
  );
  const [passingScorePercent, setPassingScorePercent] = useState(
    String(quiz?.passingScorePercent ?? 60),
  );
        ...
        idx === qi
          ? {
              ...
              options: q.options.map((o, oidx) => {
                if (oidx !== oi)
                  return o.isCorrect && patch.isCorrect && q.type === "single"
                    ? { ...o, isCorrect: false }
                    : o;
                return { ...o, ...patch };
              }),
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (invalid) {
      setError(
        "Every question needs text, at least 2 options, and at least 1 marked correct.",
      );
      setSaving(false);
      return;
    }
    ...
    <Modal title={editing ? "Edit Quiz" : "Create Quiz"} onClose={onClose} wide>
      ...
      >
        ...
        <ModalActions
          onCancel={onClose}
          submitting={saving}
          submitLabel={editing ? "Save Changes" : "Create Quiz"}
        />
      </form>
    </Modal>
  );
          ...
          {results.map((r) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className={`rounded-full px-2.5 py-1 text-xs font-bold ${r.passed ? "bg-green-lt text-green" : "bg-red-lt text-red"}`}
                >
      ...
      {quizzes && quizzes.length > 0 && (
        <div className="flex flex-col gap-3">
          {quizzes.map((q) => (
            <div
              key={q._id}
              className="flex items-center justify-between gap-3 rounded-xl border border-line bg-white p-5"
            >
              <div>
                <p className="text-xs font-medium tracking-wide text-muted uppercase">
                  {q.course?.title || "Unknown Course"} ·{" "}
                ...
                <h2 className="font-bold text-ink">{q.title}</h2>
                <p className="mt-1 text-xs text-muted">
                  {q.questions?.length ?? 0} questions · {q.timeLimitMinutes}{" "}
                  min ·{" "}
                  <span
                    className={`font-semibold ${q.status === "published" ? "text-green" : q.status === "closed" ? "text-red" : "text-muted"}`}
                  >
                    {q.status}
                  </span>
                </p>
