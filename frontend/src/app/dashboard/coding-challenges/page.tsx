...
function SolveChallenge({
  ...
  const allowed = languages.filter((l) =>
    challenge.allowedLanguages.includes(l.code),
  );
  ...
  async function handleRun() {
    ...
    setRunResult(null);
    try {
      const res = await api.post<CodeRunResult>(
        `/coding-challenges/${challenge._id}/run`,
        { language, code, stdin },
      );
      setRunResult(res);
    } catch (err) {
  ...
  async function handleSubmit() {
    ...
    setError(null);
    try {
      const res = await api.post<{ submission: CodingSubmission }>(
        `/coding-challenges/${challenge._id}/submit`,
        { language, code },
      );
        ...
        <div className="rounded-[14px] border border-line bg-white p-5">
          ...
          <div className="flex flex-col gap-2">
            {visibleTestCases.map((tc, i) => (
              <div
                key={tc._id}
                className="rounded-lg bg-bg p-3 font-mono text-xs"
              >
                <div>
                  <span className="text-muted">Input {i + 1}:</span>{" "}
                  {tc.input || "(none)"}
                </div>
                <div>
      ...
      <div className="rounded-[14px] border border-line bg-white p-5">
        ...
        <Field label="Stdin (optional, for Run only)">
          <Input
            ...
            placeholder="Custom input for a manual test run"
          />
        </Field>
        ...
        {runResult && (
          <div className="mt-3 rounded-lg bg-[#0d1117] p-3 font-mono text-xs text-[#e6edf3]">
            <div className="text-green-400">
              stdout: {runResult.stdout || "(empty)"}
            </div>
            {runResult.stderr && (
              <div className="text-red-400">stderr: {runResult.stderr}</div>
            )}
            {runResult.timedOut && (
              <div className="text-amber-400">Timed out.</div>
            )}
          </div>
        )}
          ...
          >
            <div className="flex-1">
              <div className="flex items-center justify-between">
                ...
                <span
                  className={
                    r.passed ? "font-bold text-green" : "font-bold text-red"
                  }
                >
              ...
              {!r.isHidden && !r.passed && (r.stdout || r.stderr) && (
                <div className="mt-1 rounded bg-bg p-2 font-mono text-[11px] text-ink2">
                  ...
                  {r.stderr && (
                    <div className="text-red">stderr: {r.stderr}</div>
                  )}
                </div>
              )}
            </div>
...
function StudentChallenges() {
  ...
  const [submissionsByChallenge, setSubmissionsByChallenge] = useState<
    Record<string, CodingSubmission[]>
  >({});
  const [error, setError] = useState<string | null>(null);
  const [solving, setSolving] = useState<CodingChallenge | null>(null);
  const [justSubmitted, setJustSubmitted] = useState<CodingSubmission | null>(
    null,
  );
  ...
  useEffect(() => {
    load();
    api
      .get<{ languages: CodeLanguage[] }>("/coding-challenges/languages")
    ...
    api
      ...
      .then(async (list) => {
        ...
        const entries = await Promise.all(
          list.map(async (c) => {
            try {
              const res = await api.get<{ submissions: CodingSubmission[] }>(
                `/coding-challenges/${c._id}/submissions/me`,
              );
  ...
  async function handleSolve(challengeId: string) {
    ...
    try {
      ...
      const res = await api.get<{ challenge: CodingChallenge }>(
        `/coding-challenges/${challengeId}`,
      );
      setSolving(res.challenge);
    } catch (err) {
      setError(
        err instanceof ApiError ? err.message : "Failed to load challenge",
      );
    }
  }
    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-sm text-muted">
          Practice problems, graded against real test cases.
        </p>
      </div>
      ...
      {challenges && challenges.length === 0 && (
        <EmptyState message="No coding challenges assigned yet." />
      )}
        ...
        <div className="flex flex-col gap-3">
          {challenges.map((c) => {
            ...
            const best = submissions.reduce(
              (b, s) => (!b || s.percentage > b.percentage ? s : b),
              null as CodingSubmission | null,
            );
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    <p className="text-xs font-medium tracking-wide text-muted uppercase">
                      ...
                      {c.batch?.name || "Unknown Batch"}
                    </p>
                    <h2 className="font-bold text-ink">{c.title}</h2>
                    <p className="mt-1 text-xs text-muted">
                      {c.testCases?.length ?? "?"} test cases · {attemptsLeft}{" "}
                      of {c.maxAttempts} attempts left
                    </p>
                  </div>
                  {c.status === "published" && attemptsLeft > 0 ? (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="shrink-0 rounded-full bg-purple px-5 py-2.5 text-[0.85rem] font-bold text-white transition-all hover:bg-purple-dk active:scale-95 text-center inline-block w-full sm:w-auto"
                    >
                      Solve Challenge ↗
                    </a>
                  ) : (
                ...
                {best && (
                  <p className="mt-3 rounded-lg bg-bg p-3 text-sm text-ink2">
                    Best score:{" "}
                    <span
                      className={`font-bold ${best.passed ? "text-green" : "text-red"}`}
                    >
                      {best.score}/{best.maxScore} ({best.percentage}%)
                    </span>
                  </p>
                )}
              </div>
...
function ChallengeModal({
  ...
  const [maxAttempts, setMaxAttempts] = useState(
    String(challenge?.maxAttempts ?? 5),
  );
  const [status, setStatus] = useState(challenge?.status ?? "published");
  const [testCases, setTestCases] = useState<DraftTestCase[]>(
    challenge?.testCases.length
      ? challenge.testCases.map((tc: TestCase) => ({
          input: tc.input,
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const finalTestCases =
        testCases.length > 0 && testCases[0].expectedOutput.trim()
          ? testCases
          : [
              {
                input: "dummy",
      ...
      const payload = {
        ...
        status: "published",
        testCases: finalTestCases.map((tc) => ({
          input: tc.input,
          expectedOutput: tc.expectedOutput,
          isHidden: tc.isHidden,
          points: tc.points,
      ...
      if (editing && challenge) {
        ...
        await api.post("/coding-challenges", {
          ...payload,
          batch: batchId,
          course: courseId,
        });
      ...
      setError(
        err instanceof ApiError ? err.message : "Failed to save challenge",
      );
      setSaving(false);
    }
  }
    ...
    >
      ...
      >
        ...
        <Field
          label="Description"
          required
          hint="Problem statement — shown to students."
        >
          <Textarea
            value={description}
        ...
        <ModalActions
          onCancel={onClose}
          submitting={saving}
          submitLabel={editing ? "Save Changes" : "Create Challenge"}
        />
      </form>
    </Modal>
  );
...
function SubmissionsModal({
  ...
  const [submissions, setSubmissions] = useState<CodingSubmission[] | null>(
    null,
  );
  ...
  useEffect(() => {
    api
      .get<{ submissions: CodingSubmission[] }>(
        `/coding-challenges/${challenge._id}/submissions`,
      )
            ...
            return (
              <div key={s._id} className="py-2.5">
                ...
                >
                  ...
                  <span
                    className={`rounded-full px-2.5 py-1 text-xs font-bold ${s.passed ? "bg-green-lt text-green" : "bg-red-lt text-red"}`}
                  >
                ...
                {isOpen && (
                  <div className="mt-2 rounded-lg bg-[#0d1117] p-3">
                    <pre className="max-h-72 overflow-auto whitespace-pre-wrap font-mono text-xs text-[#e6edf3]">
                      {s.code}
                    </pre>
                  </div>
                )}
...
function MentorAdminChallenges() {
  ...
  async function handleDelete(c: CodingChallenge) {
    if (!confirm(`Delete challenge "${c.title}"? This cannot be undone.`))
      return;
    try {
      ...
      setError(
        err instanceof ApiError ? err.message : "Failed to delete challenge",
      );
    }
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {error && <ErrorText>{error}</ErrorText>}
      {challenges && challenges.length === 0 && (
        <EmptyState message="No coding challenges yet." />
      )}
      ...
      {challenges && challenges.length > 0 && (
        <div className="flex flex-col gap-3">
          {challenges.map((c) => (
            <div
              key={c._id}
              className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 rounded-xl border border-line bg-white p-5"
            >
              <div>
                <p className="text-xs font-medium tracking-wide text-muted uppercase">
                  {c.course?.title || "Unknown Course"} ·{" "}
                ...
                <p className="mt-1 text-xs text-muted">
                  ...
                  <span
                    className={`font-semibold ${c.status === "published" ? "text-green" : c.status === "closed" ? "text-red" : "text-muted"}`}
                  >
                    {c.status}
                  </span>
                </p>
              </div>
              <div className="flex flex-wrap w-full sm:w-auto shrink-0 items-center gap-2">
                <SecondaryButton
                  type="button"
                  onClick={() => setViewingSubmissions(c)}
