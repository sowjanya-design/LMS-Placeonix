...
function Player() {
  ...
  if (!uid) {
    return (
      <div className="p-8 text-center text-red-600">Video UID not found.</div>
    );
  }
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="aspect-video w-full max-w-4xl mx-auto rounded-lg overflow-hidden border border-line bg-black relative flex items-center justify-center">
        {uid.startsWith("local_video_") ? (
          <video
            src={`${process.env.NEXT_PUBLIC_API_BASE?.replace('/api/v1', '') || 'https://backend-pearl-seven-77.vercel.app'}/uploads/videos/${uid.replace("local_video_", "")}`}
            controls
          ...
          <div className="text-center text-white flex flex-col items-center gap-4 p-8">
            ...
            <p className="text-sm text-gray-400 max-w-md">
              This is a simulated video player. You uploaded a video without
              Cloudflare Stream configured. If you want local playback, make
              sure your backend is running.
            </p>
          </div>
        ) : (
          <iframe
...
export default function VideoPlayerPage() {
  return (
    <Suspense
      fallback={<div className="p-8 text-center">Loading player...</div>}
    >
      <Player />
    </Suspense>
  );
