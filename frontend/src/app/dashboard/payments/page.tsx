              ...
              return (
                <div key={e._id} className="rounded-2xl border border-line bg-white p-5 shadow-sm flex flex-col gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-bold text-ink">{course?.title}</div>
                      <div className="text-xs text-muted uppercase tracking-wide">{course?.category?.replace("_", " ")}</div>
                    </div>
                  </div>
                  <div className="flex items-center justify-between rounded-xl bg-bg px-4 py-3">
                    ...
                    <span className="font-bold text-ink">₹{(e.fee?.total ?? course?.fee?.amount ?? 0).toLocaleString("en-IN")}</span>
                  </div>
                  <div className="flex items-center justify-between rounded-xl bg-bg px-4 py-3">
                    <span className="text-sm text-muted">Paid</span>
                    <span className="font-semibold text-green">₹{(e.fee?.paid ?? 0).toLocaleString("en-IN")}</span>
                  </div>
                  <div className="flex items-center justify-between rounded-xl bg-bg px-4 py-3">
                    <span className="text-sm text-muted">Due</span>
                    <span className={`font-bold ${e.fee?.due ? "text-red" : "text-green"}`}>₹{(e.fee?.due ?? 0).toLocaleString("en-IN")}</span>
                  </div>
                  <div className="flex items-center justify-between rounded-xl bg-bg px-4 py-3">
