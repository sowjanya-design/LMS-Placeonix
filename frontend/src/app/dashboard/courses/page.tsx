...
import { Field, Input, Select, Textarea, ModalActions, ErrorText, PrimaryButton } from "@/components/ui/form";
...
const CATEGORIES = ["Web Development", "Data Science", "ERP", "Cloud & DevOps", "Cybersecurity", "UI/UX", "Programming", "Other"] as const;
const LEVELS = ["Beginner", "Intermediate", "Advanced", "Beginner to Advanced"] as const;
...
function CourseModal({ course, onClose, onSaved }: { course: Course | null; onClose: () => void; onSaved: (c: Course) => void }) {
  const isEdit = !!course;
  const [form, setForm] = useState<CourseForm>(() => toForm(course ?? undefined));
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.title.trim() || !form.duration.trim() || !form.description.trim() || !Number.isFinite(feeAmount) || feeAmount < 0) {
    ...
    try {
      ...
      const res = isEdit
        ? await api.patch<{ course: Course }>(`/courses/${course!._id}`, payload)
        : await api.post<{ course: Course }>("/courses", payload);
      onSaved(res.course);
      onClose();
    } catch (err) {
      setError(err instanceof ApiError ? err.message : `Failed to ${isEdit ? "update" : "create"} course`);
    } finally {
      setSubmitting(false);
    }
  }
    ...
    <Modal title={isEdit ? "Edit Course" : "Add Course"} onClose={onClose} wide>
      <form onSubmit={handleSubmit} className="flex flex-col gap-3">
        <Field label="Title" required>
          <Input value={form.title} onChange={(e) => set("title", e.target.value)} placeholder="Full Stack Web Development" />
        </Field>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          ...
          <Field label="Duration" required hint='e.g. "4 Months"'>
            <Input value={form.duration} onChange={(e) => set("duration", e.target.value)} placeholder="4 Months" />
          </Field>
        </div>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-[1fr_auto]">
          <Field label="Fee (₹)" required>
            <Input type="number" min={0} value={form.feeAmount} onChange={(e) => set("feeAmount", e.target.value)} placeholder="25000" />
          </Field>
          <Field label="Card color" hint="Used as this course's tag color across the app">
            <input
              type="color"
              value={form.color}
        ...
        <Field label="Short description" hint="Shown on the course card">
          <Input value={form.shortDescription} onChange={(e) => set("shortDescription", e.target.value)} placeholder="One line about the course" />
        </Field>
        <Field label="Description" required>
          <Textarea rows={4} value={form.description} onChange={(e) => set("description", e.target.value)} placeholder="Full course description" />
        </Field>
        <label className="flex items-center gap-2 text-sm font-semibold text-ink2">
          <input type="checkbox" checked={form.isPublished} onChange={(e) => set("isPublished", e.target.checked)} className="accent-purple" />
          Published — visible to students and in the public catalog
        </label>
        <ErrorText>{error}</ErrorText>
        <ModalActions onCancel={onClose} submitting={submitting} submitLabel={isEdit ? "Save changes" : "Create course"} />
      </form>
    </Modal>
  );
}
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {filtered.map((c) => (
              <div key={c._id} className="flex flex-col gap-3 rounded-[14px] border border-line bg-white p-5">
                <div>
                  <div className="flex items-center justify-between">
                    <div className="text-xs font-bold tracking-wide uppercase" style={{ color: c.color || "var(--purple)" }}>
                      {c.category}
                    </div>
                    {isAdmin && (
                      <span className={`rounded-full px-2 py-0.5 text-[0.65rem] font-bold ${c.isPublished ? "bg-green-lt text-green" : "bg-amber-lt text-amber"}`}>
                ...
                <div className="mt-auto flex items-center justify-between border-t border-line pt-3 text-xs text-muted">
                  ...
                  {c.fee?.amount != null && <span className="font-semibold text-ink2">₹{c.fee.amount.toLocaleString("en-IN")}</span>}
                </div>
                {isAdmin && (
            ...
            {filtered.length === 0 && <EmptyState message="No courses in this category." className="col-span-full" />}
          </div>
        </>
      )}

      {isAdmin && showAdd && <CourseModal course={null} onClose={() => setShowAdd(false)} onSaved={upsert} />}
      {isAdmin && editing && <CourseModal course={editing} onClose={() => setEditing(null)} onSaved={upsert} />}
    </div>
  );
}
