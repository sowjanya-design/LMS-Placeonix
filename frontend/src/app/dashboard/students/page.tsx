...
function AddStudentModal({
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (form.batchId) {
        try {
          await api.post(`/batches/${form.batchId}/enroll`, {
            studentId: res.user._id,
          });
        } catch (enrollErr) {
          console.error("Failed to enroll new student:", enrollErr);
          alert(
            "Student created, but batch enrollment failed. You can assign them manually in the Edit screen.",
          );
        }
      }
      onAdded(res.user);
  ...
  return (
    <Modal title="Add Student" onClose={onClose}>
      ...
      >
        ...
        <Field
          label="Assign Batch & Course"
          hint="Enroll immediately upon creation (optional)"
        >
          <Select
            value={form.batchId}
...
function EditStudentModal({
  ...
  useEffect(() => {
    api
      .get<Enrollment[]>(`/users/${student._id}/enrollments`)
      .then(setEnrollments)
  ...
  async function handleEnroll() {
    ...
    try {
      await api.post(`/batches/${enrollingBatch}/enroll`, {
        studentId: student._id,
      });
      const newEnrollments = await api.get<Enrollment[]>(
        `/users/${student._id}/enrollments`,
      );
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const res = await api.patch<{ user: User }>(`/users/${student._id}`, {
        ...form,
      ...
      setError(
        err instanceof ApiError ? err.message : "Failed to update student",
      );
    } finally {
      setSubmitting(false);
    }
        ...
        <div className="flex flex-col gap-2">
          <Field label="Enroll in New Batch">
            <div className="flex gap-2">
              <Select
                value={enrollingBatch}
                onChange={(e) => setEnrollingBatch(e.target.value)}
...
export default function StudentsPage() {
  ...
  async function handleDelete(u: User) {
    if (
      !confirm(
        `Remove ${u.firstName} ${u.lastName}? This also deletes their enrollments.`,
      )
    )
      return;
    setDeletingId(u._id);
    ...
    <div className="flex flex-col gap-6">
              ...
              {students.map((s) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`rounded-md px-2.5 py-1 text-xs font-semibold ${STATUS_STYLE[s.status] ?? STATUS_STYLE.active}`}
                      >
                        {s.status}
                      </span>
                    </td>
      ...
      {editing && (
        <EditStudentModal
          ...
          onUpdated={(u) =>
            setStudents(
              (prev) => prev?.map((s) => (s._id === u._id ? u : s)) ?? prev,
            )
          }
        />
      )}
