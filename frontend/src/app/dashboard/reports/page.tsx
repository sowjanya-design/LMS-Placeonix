...
function exportCsv(rows: ReportRow[]) {
  ...
  downloadBlob([header, ...lines].join("\n"), `placeonix-report-${new Date().toISOString().slice(0, 10)}.csv`, "text/csv;charset=utf-8;");
}
...
async function exportPdf(rows: ReportRow[]) {
  ...
  doc.text(new Date().toLocaleDateString(undefined, { year: "numeric", month: "long", day: "numeric" }), 14, 22);
  autoTable(doc, {
    startY: 28,
    ...
    body: rows.map(([s, l, v]) => [s.replace(/—/g, "-"), l, String(v).replace(/₹/g, "Rs. ")]),
  ...
  const rows: ReportRow[] = [
    ...
    ...(placement
      ? ([
          ...
          ...placement.funnel.map((f) => ["Placements — Funnel", f.label, f.count] as ReportRow),
        ] as ReportRow[])
      : []),
    ...(revenue
      ? ([
          ["Revenue", "Total Committed", `₹${revenue.summary.totalCommitted.toLocaleString("en-IN")}`],
          ["Revenue", "Collected", `₹${revenue.summary.totalRevenue.toLocaleString("en-IN")}`],
          ["Revenue", "Outstanding", `₹${revenue.summary.totalDue.toLocaleString("en-IN")}`],
        ] as ReportRow[])
      : []),
  ];
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        {rows.length > 0 && (
          <div className="flex gap-2">
            <SecondaryButton onClick={() => exportCsv(rows)}>Export CSV</SecondaryButton>
            <SecondaryButton onClick={() => exportPdf(rows)}>Export PDF</SecondaryButton>
          </div>
        )}
      </div>
