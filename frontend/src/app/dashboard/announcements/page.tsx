...
export default function AnnouncementsPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {canPost && (
        <NewAnnouncementForm
          onCreated={(a) => setItems((prev) => (prev ? [a, ...prev] : [a]))}
        />
      )}
            ...
            >
              <div className="flex items-start justify-between gap-2">
                ...
                <div className="flex items-center gap-2">
                  <span
                    className={`rounded-full px-2.5 py-1 text-xs font-semibold ${TYPE_STYLE[a.type]}`}
                  >
                    {a.type}
                  </span>
                  {a.isSystemHoliday && (
                  ...
                  {canPost && !a.isSystemHoliday && (
                    <button
                      ...
                      className="text-xs font-semibold text-purple hover:text-purple-dk"
                      aria-label="Edit"
                    >
                      Edit
                    </button>
      ...
      {editing && (
        <EditAnnouncementForm
          ...
          onSaved={(updated) =>
            setItems(
              (prev) =>
                prev?.map((x) => (x._id === updated._id ? updated : x)) ?? prev,
            )
          }
        />
      )}
